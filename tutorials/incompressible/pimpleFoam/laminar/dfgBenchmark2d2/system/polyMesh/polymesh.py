...
class PolyMesh:
    ...
    def read(cls, case: Optional[Path] = None):
        ...
        neighbour = cls.read_index(case, "neighbour")
        owner = cls.read_index(case, "owner")
        return cls(
    ...
    @staticmethod
    def read_foamfile(fp: TextIO) -> None:
        """read the FoamFile boilerplate
        ...
        with open(cls.polymesh(case) / "boundary", "r") as fin:
            ...
            for _ in range(npatches):
                ...
                for line in fin:
                    ...
                    if words[0] == "}":
                        break
                    elif words[0] in {"startFace", "nFaces"}:
                        termini[words[0]] = int(words[1])
                boundary[name] = slice(
                    termini["startFace"], termini["startFace"] + termini["nFaces"]
                )

        return boundary

    @classmethod
    def read_index(cls, case: Path, name: str) -> np.ndarray:
    ...
    def to_meshquad(
        ...
        front_points, inverse = np.unique(front_faces, return_inverse=True)
        ...
        if transform:
            points = points @ np.hstack([transform, np.zeros((3, 1))])
        return meshio.Mesh(
            points,
